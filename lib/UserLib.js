...
const UserLib = {
  ...
  async dropIndexOne(data) {
    try {
      const experienceRes = await User.collection.dropIndex({
        'email': 1
      })
      if(!experienceRes) {
  ...
  async getIndexx() {
    try {
      const experienceRes = await User.collection.getIndexes()
      if(!experienceRes) {
        throw new Error('No Experiences Found!')
      } 
      debug.info('Experiences: ', experienceRes)
