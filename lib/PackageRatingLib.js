...
const PackageRatingLib = {
  ...
  async getReviewCount (package_id) {
    try {
      const packageRatingRes = await PackageRating.find({
        package_id: package_id
      }).count()
      if(!packageRatingRes) {
        throw new Error('No ID Found or ERROR: updating Package!')
      }
      debug.info('Package Updated Result', packageRatingRes)
      return packageRatingRes
    } catch (error) {
      debug.error("ERROR: Found in updating Package!", error)
      return
    }
  },
